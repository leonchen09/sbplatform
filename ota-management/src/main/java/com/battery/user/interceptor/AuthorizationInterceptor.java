package com.battery.user.interceptor;
...
//		response.setHeader("Access-Control-Allow-Headers", "content-type,authorization");
		...
		if (authorization != null && !authorization.trim().equals("")) {
			...
			if (resultMap.get("code").toString().equals("0")) {
				request.setAttribute(Constant.REQUEST_USER_ID, resultMap.get("aud"));
				request.setAttribute(Constant.REQUEST_USER_TYPE, resultMap.get("type"));
				...
				if (resultMap.get("code").toString().equals("9") && requestUrl.equals("/token/renew")) {
					request.setAttribute(Constant.REQUEST_USER_ID, resultMap.get("aud"));
					request.setAttribute(Constant.REQUEST_USER_TYPE, resultMap.get("type"));
					return true;
				}
